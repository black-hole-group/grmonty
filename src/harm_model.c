		    ...
		    double Ucon[NDIM], double Bcon[NDIM])
{
	...
	*Ne = p[KRHO][SPATIAL_INDEX3D(i,j,k)] * Ne_unit;
	*Thetae = p[UU][SPATIAL_INDEX3D(i,j,k)] / (*Ne) * Ne_unit * Thetae_unit;

	Bp[1] = p[B1][SPATIAL_INDEX3D(i,j,k)];
	Bp[2] = p[B2][SPATIAL_INDEX3D(i,j,k)];
	Bp[3] = p[B3][SPATIAL_INDEX3D(i,j,k)];

	Vcon[1] = p[U1][SPATIAL_INDEX3D(i,j,k)];
	Vcon[2] = p[U2][SPATIAL_INDEX3D(i,j,k)];
	Vcon[3] = p[U3][SPATIAL_INDEX3D(i,j,k)];

	/* Get Ucov */
	VdotV = 0.;
	for (l = 1; l < NDIM; l++)
		for (m = 1; m < NDIM; m++)
			VdotV += geom[SPATIAL_INDEX2D(i,j)].gcov[l][m] * Vcon[l] * Vcon[m];
	Vfac = sqrt(-1. / geom[SPATIAL_INDEX2D(i,j)].gcon[0][0] * (1. + fabs(VdotV)));
	Ucon[0] = -Vfac * geom[SPATIAL_INDEX2D(i,j)].gcon[0][0];
	for (l = 1; l < NDIM; l++){
		Ucon[l] = Vcon[l] - Vfac * geom[SPATIAL_INDEX2D(i,j)].gcon[0][l];
	}
	...
	*B = sqrt(Bcon[0] * Bcov[0] + Bcon[1] * Bcov[1] +
	...
	if (isnan(*B)){
		...
		fprintf(stderr, "Vfac = %lf\n", Vfac);
		for(int a = 0; a < NDIM; a++) for(int b=0;b<NDIM;b++)fprintf(stderr, "gcon[%d][%d]: %lf\n", a, b, geom[SPATIAL_INDEX2D(i,j)].gcon[a][b]);
		for(int a = 0; a < NDIM; a++) for(int b=0;b<NDIM;b++)fprintf(stderr, "gcov[%d][%d]: %lf\n", a, b, geom[SPATIAL_INDEX2D(i,j)].gcov[a][b]);
		fprintf(stderr, "Thetae: %lf\n", *Thetae);
		fprintf(stderr, "Ne: %lf\n", *Ne);
		fprintf(stderr, "Bp: %lf, %lf, %lf\n", Bp[1], Bp[2], Bp[3]);
		fprintf(stderr, "Vcon: %lf, %lf, %lf\n", Vcon[1], Vcon[2], Vcon[3]);
		fprintf(stderr, "Bcon: %lf, %lf, %lf, %lf\n Bcov: %lf, %lf, %lf %lf\n", Bcon[0], Bcon[1], Bcon[2], Bcon[3], Bcov[0], Bcov[1], Bcov[2], Bcov[3]);
		fprintf(stderr, "Ucon: %lf, %lf, %lf, %lf\n Ucov: %lf, %lf, %lf %lf\n", Ucon[0], Ucon[1], Ucon[2], Ucon[3], Ucov[0], Ucov[1], Ucov[2], Ucov[3]);
	}
